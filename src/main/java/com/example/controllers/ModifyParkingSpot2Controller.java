...
public class ModifyParkingSpot2Controller implements Initializable {
    private Stage stage;
    private Scene scene;
    private Parent root;
    ...
    void onAddButtonClick(ActionEvent event) throws Exception {
        ...
        ArrayList<Integer> myList = getParcare2Number(getConnection());
        try{
            ...
            String sql = "INSERT INTO parcare2(sector,number,isoccupied) VALUES(?,?,?)";
            PreparedStatement ps = connection.prepareStatement(sql);
            ps.setString(1,value1);
            ps.setString(2,value2);
            ps.setString(3,value3Better);
            ps.executeUpdate();
            parcare1Table.refresh();
        }catch (Exception e){
    ...
    void onDeleteButtonClick(ActionEvent event) throws Exception {
        ...
        try{
            ...
            String sql = "DELETE FROM parcare2 where sector = ? AND number = ?";
            PreparedStatement ps = connection.prepareStatement(sql);
            ps.setString(1,value1);
            ps.setString(2,value2);
            ps.executeUpdate();
    ...
    @FXML
    void onUpdateButtonClick(ActionEvent event) throws Exception {
        Connection connection = BazaDeDate.getConnection();
        try{
            String value1 = sectorField.getText();
            String value2  = numberField.getText();
            ...
            String sql = "UPDATE parcare2 set isoccupied = ? WHERE sector = ? and number = ? ";
            PreparedStatement ps = connection.prepareStatement(sql);
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            ...
            parkingList = getDataUsersFromTable2(getConnection());
    ...
    void onParcare2ButtonClicked(ActionEvent event) {
        try {
            URL url = Paths.get("C:\\Users\\Lenovo\\IdeaProjects\\SmartParkingApp\\src\\main\\resources\\com\\example\\smartparkingapp\\modify-parkingSpot2-view.fxml").toUri().toURL();
            FXMLLoader loader = new FXMLLoader(url);
            root = loader.load();
    ...
    void onParcare3ButtonClicked(ActionEvent event) {
        try {
            URL url = Paths.get("C:\\Users\\Lenovo\\IdeaProjects\\SmartParkingApp\\src\\main\\resources\\com\\example\\smartparkingapp\\modify-ParkingSpot3-view.fxml").toUri().toURL();
            FXMLLoader loader = new FXMLLoader(url);
            root = loader.load();
